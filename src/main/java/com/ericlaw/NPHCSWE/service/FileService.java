...
public class FileService implements IFileService {
  ...
  public void uploadUsersCSVFile(MultipartFile file) {
    ...
    if (!isCSVFile(file.getOriginalFilename())) throw new FileException(
      CustomException.INVALID_FILE
    );

    if (isEmptyFile(file)) throw new FileException(CustomException.EMPTY_FILE);
    ...
    try {
      ...
      while ((line = csvReader.readNext()) != null) {
        //Skip line if first character is #
        if (Character.compare(line[0].charAt(0), '#') != 0) {
    ...
    if (hasDuplicateIDInFile(users)) throw new FileException(
      CustomException.DUPLICATE_ID
    );

    if (hasDuplicateLoginInFile(users)) throw new FileException(
  ...
  private String checkLineForErrors(String[] line, Long currentLine) {
    ...
    if (line[0].isEmpty()) {
      ...
      line[3].isEmpty() || bigDecimalValidator.validate(line[3]) == null
    ) {
      message =
        String.format(
      ...
      message =
        String.format("Missing or invalid date value on line %s.", currentLine);
    }
